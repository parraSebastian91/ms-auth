...
export class UsuarioRepositoryAdapter implements IUsuarioRepository {
    ...
    async createUsuario(data: RegistroUsuarioModel): Promise<{ usuarioUuid: string }> {
        const query = `
        ...
        `
        try {
            const respuesta = await this.usuarioRepository.query(query, [data.username, data.passwordHash, data.contactoid]);
    ...
    async getUsuarioByEmail(email: string): Promise<{ id: number; uuid: string; emailVerificado: boolean; nombres: string } | null> {
    ...
    async validateField(field: string, value: string): Promise<{ available: boolean; message?: string }> {
        ...
        try {
            ...
            console.log(`Validation result for field ${field} with value ${value}:`, result);
            if (result[0].available) {
                return { available: true, message: `${nombreCampo} está disponible` };
            }
            return { available: false, message: `${nombreCampo} ya existe` };
        } catch (error) {
            this.logger.error(`Error validating field ${field} with value ${value}: ${error}`);
