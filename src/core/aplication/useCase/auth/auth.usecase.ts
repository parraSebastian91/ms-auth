...
} from './command/AuthCommand.interface';
...
export class AuthUseCase implements IAuthUseCase {
  ...
  constructor(
    ...
    this.accessExpiresIn = this.configService.get<string>(
      'jwtConfig.access_expires_in',
    );
    this.adminExpiresIn = this.configService.get<string>(
      'jwtConfig.admin_expires_in',
    );
    this.refreshExpiresIn = this.configService.get<string>(
      'jwtConfig.refresh_expires_in',
    );
  }

  async ExcuteAuthentication(
    command: AuthenticationCommand,
  ): Promise<{ code: string; url: string }[] | null> {
    const requestId = command.requestId || 'N/A';
    this.logger.log(
      `[AUTHENTICATE] INIT requestId=${requestId} username=${command.username} device=${command.typeDevice} CorrelationId=${command.CorrelationId}`,
    );
    ...
    if (!usuario) {
      this.logger.warn(
        `[AUTHENTICATE] USER_NOT_FOUND requestId=${requestId} username=${command.username} CorrelationId=${command.CorrelationId}`,
      );
      throw new UserNotFoundError('Usuario no encontrado');
    }
    if (!(await bcrypt.compare(command.password, usuario.password))) {
      this.logger.warn(
        `[AUTHENTICATE] INVALID_CREDENTIALS requestId=${requestId} username=${command.username} CorrelationId=${command.CorrelationId}`,
      );
    ...
    if (!usuario.emailVerificado) {
      ...
      this.logger.warn(
        `[AUTHENTICATE] EMAIL_NOT_VERIFIED requestId=${requestId} username=${command.username} CorrelationId=${command.CorrelationId}`,
      );
      throw new EmailNotVerifiedError(email);
    }
    this.logger.log(
      `[AUTHENTICATE] CREDENTIALS_VALID requestId=${requestId} userUuid=${usuario.uuid} CorrelationId=${command.CorrelationId}`,
    );
    ...
    this.logger.log(
      `[AUTHENTICATE] AUTH_CODE_CREATED requestId=${requestId} userUuid=${usuario.uuid} CorrelationId=${command.CorrelationId}`,
    );
    ...
    this.logger.log(
      `[AUTHENTICATE] SUCCESS requestId=${requestId} username=${command.username} systems=${uris.length}`,
    );
    return uris;
  }
  ...
  async ExecuteAuthorization(
    ...
    const requestId = command.requestId || 'N/A';
    this.logger.log(
      `[AUTHORIZATION] INIT requestId=${requestId} sessionId=${command.sessionId} device=${command.typeDevice} CorrelationId=${command.CorrelationId}`,
    );
    ...
    this.logger.log(
      `[AUTHORIZATION] AUTH_CODE_FOUND requestId=${requestId} CorrelationId=${stored.CorrelationId} userUuid=${stored.userUuid} CorrelationId=${command.CorrelationId}`,
    );
    ...
    if (incomingChallenge !== stored.codeChallenge) {
      this.logger.warn(
        `[AUTHORIZATION] INVALID_PKCE requestId=${requestId} sessionId=${command.sessionId} CorrelationId=${command.CorrelationId}`,
      );
    ...
    if (storedDevice !== commandDevice) {
      this.logger.warn(
        `[AUTHORIZATION] DEVICE_MISMATCH requestId=${requestId} stored=${storedDevice} incoming=${commandDevice} sessionId=${command.sessionId} CorrelationId=${command.CorrelationId}`,
      );
    ...
    this.logger.log(
      `[AUTHORIZATION] AUTH_CODE_DELETED requestId=${requestId} sessionId=${command.sessionId} CorrelationId=${command.CorrelationId}`,
    );
    const tokens = await this.authService.createRefreshSession(stored);
    this.logger.log(
      `[AUTHORIZATION] SUCCESS requestId=${requestId} sessionId=${command.sessionId} userUuid=${stored.userUuid} CorrelationId=${command.CorrelationId}`,
    );
    return tokens;
  }
  ...
  async ExecuteRequestPasswordRequest(
  ...
  async ExecuteRequestPasswordValidation(
